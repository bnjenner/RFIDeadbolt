...
RFID::RFID(int chipSelectPin, int NRSTPD, int sckPin, int mosiPin, int misoPin)
{
  ...
  // Software SPI: the 32U4 exposes hardware SPI only on the ICSP header, so the
  // bus is bit-banged on three ordinary digital pins instead.
  _sckPin  = sckPin;
  _mosiPin = mosiPin;
  _misoPin = misoPin;
  ...
  digitalWrite(_sckPin, LOW);				// clock idles low (CPOL = 0)
  digitalWrite(_mosiPin, LOW);
}

/*
 * Shift one byte out on MOSI while shifting one byte in on MISO.
 * SPI mode 0 (CPOL = 0, CPHA = 0), MSB first, as the MFRC522 expects: MOSI is
 * set while the clock is low, and both ends sample on the rising edge.
 */
unsigned char RFID::transfer(unsigned char data)
{
	unsigned char in = 0;
